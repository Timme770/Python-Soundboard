...
#
#    Authors: Tim H. <contact at https://discord.gg/8hRXDnM (Timme770#6693), https://www.twitch.tv/tuxplayde>
...
def play(hk):
    try:
        ...
        print("Variable "+hotkeypressed+" wurde nicht gefundenoder es ist etwas in der funktion 'def play(hk) schiefgelaufen'")

def startlisten(hk):
    for i in hk:
        #test=keyboard.add_hotkey(i[1], lambda: play(hk))
        root.listenkeyeventhandlerliste.append(   keyboard.add_hotkey(i[1], lambda: play(hk))   )
        # TODO Debugzeugs wieder entfernen!
    print("listen EventHandlerListe:\n" + str(root.listenkeyeventhandlerliste)  )
...
def volumeset(event):
    print("\n Volumewert: " + str(event))
    print("Leiser!!") # TODO do something! slider und so
    if root.player == None:
        return
    volume = int(event)
    if volume > 100:
...
def listendoppelklick_hotkey(event):
    ...
    if root.hotkeyAufnahme == "":                                                                                                               # wurde kein Hotkey im Dialog eingegeben?
        ...
        print("listbox-size: " + str(listbox_hk.size()) + "  |||  auswahlnummer: " + str(auswahl))
        if listbox_hk.size() == ( auswahl + 1 ):                                                        # Der Eintrag für ein neues Element in der Liste wurde geändert/gewaehlt
            ...
            listbox_dn.delete(auswahl)                                                                  # Eintrag in der Dateinamen Liste aktualisieren auf ein leeren inhalt (ist ja komplett neu)
            listbox_dn.insert(auswahl, "")                                                              # ^^^
            listboxenneuereintraghinzufuegen()                                                          # Neuen Platzhalter fuer hinzufuegen erstellen.
        else:
            root.hotkeys[auswahl]=((root.hotkeys[auswahl][0]), str(root.hotkeyAufnahme))                # die Tulpe/von der config gelesene liste auch aktualisieren

def listendoppelklick_dateiname(event):
    auswahl=listbox_dn.curselection()[0]                                                                                                        # den ausgewählten eintrag in der listbox auslesen
    try:
        root.neuedatei= filedialog.askopenfilename(initialdir = "~/",title = "Eine Vlc kompatible Datei auswählen",filetypes = [("alle Dateien", "*.*")])#"Audiodateien","*.mp3"),("all files","*.*")))     #(angabe aller audioformate wäre ganz schön mühsam, villeicht später # TODO)
        ...
        if listbox_dn.size() == ( auswahl + 1 ):                                                        # Der Eintrag für ein neues Element in der Liste wurde geändert/gewaehlt
            ...
            listbox_hk.delete(auswahl)                                                                  # Eintrag in der Dateinamen Liste aktualisieren auf ein leeren inhalt (ist ja komplett neu)
            listbox_hk.insert(auswahl, "")                                                              # ^^^
            listboxenneuereintraghinzufuegen()                                                          # Neuen Platzhalter fuer hinzufuegen erstellen.
        else:
            root.hotkeys[auswahl]=(str(root.neuedatei),root.hotkeys[auswahl][1])                        # die Tulpe/von der config gelesene liste auch aktualisieren
    except:
...
################ top frame  [ Config / Speichern ]
   ...
   reloadButton     =   Button(  frame_saveandreload, text="Config neu Laden"  , command=lambda: reloadconfiganddisplay(configurationsdatei)     )
